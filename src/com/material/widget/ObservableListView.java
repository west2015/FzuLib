...
import com.material.widget.scroll.*;
...
public class ObservableListView extends ListView implements Scrollable {
	...
	private OnScrollListener mScrollListener = new OnScrollListener() {
		...
		public void onScroll(AbsListView view, int firstVisibleItem,
			...
			if (mOriginalScrollListener != null) {
				mOriginalScrollListener.onScroll(view, firstVisibleItem,
						visibleItemCount, totalItemCount);
			}
			// AbsListView#invokeOnItemScrollListener calls onScrollChanged(0,
			// 0, 0, 0)
		...
		if (mCallbacks != null) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (getCurrentScrollY() - diffY <= 0) {
					...
					// Apps can set the interception target other than the
					// direct parent.
					...
					float offsetX = 0;
					float offsetY = 0;
					for (View v = this; v != null && v != parent; v = (View) v
							.getParent()) {
			...
			if (getChildCount() > 0) {
				...
				for (int i = getFirstVisiblePosition(), j = 0; i <= getLastVisiblePosition(); i++, j++) {
					if (mChildrenHeights.indexOfKey(i) < 0
							|| getChildAt(j).getHeight() != mChildrenHeights
									.get(i)) {
				...
				if (firstVisibleChild != null) {
					if (mPrevFirstVisiblePosition < firstVisiblePosition) {
						...
						if (firstVisiblePosition - mPrevFirstVisiblePosition != 1) {
							for (int i = firstVisiblePosition - 1; i > mPrevFirstVisiblePosition; i--) {
								if (0 < mChildrenHeights.indexOfKey(i)) {
									skippedChildrenHeight += mChildrenHeights
											.get(i);
								} else {
									// Approximate each item's height to the
									// first visible child.
									// It may be incorrect, but without this,
									// scrollY will be broken
									// when scrolling from the bottom.
									skippedChildrenHeight += firstVisibleChild
											.getHeight();
								}
							}
						}
						...
						mPrevFirstVisibleChildHeight = firstVisibleChild
								.getHeight();
						...
						mPrevScrolledChildrenHeight -= firstVisibleChild
								.getHeight() + skippedChildrenHeight;
						mPrevFirstVisibleChildHeight = firstVisibleChild
								.getHeight();
					...
					mScrollY = mPrevScrolledChildrenHeight
							- firstVisibleChild.getTop();
					mPrevFirstVisiblePosition = firstVisiblePosition;

					mCallbacks.onScrollChanged(mScrollY, mFirstScroll,
							mDragging);
					if (mFirstScroll) {
						mFirstScroll = false;
					}
	...
	static class SavedState extends BaseSavedState {
		...
		public void writeToParcel(Parcel out, int flags) {
			...
			final int numOfChildren = childrenHeights == null ? 0
					: childrenHeights.size();
